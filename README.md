...
#### ステップ3: グローバル設定を確認
`~/.claude/CLAUDE.md`が既に設定済みです。どのディレクトリでも`claude-code`を起動すると、この設定が適用されます。

**メリット**: 一度設定すれば、全てのプロジェクトで統一された設定が使用できます。
...
プロジェクト毎に独自の設定が必要な場合は、そのプロジェクトに`CLAUDE.md`を配置することで、グローバル設定を上書きできます。
